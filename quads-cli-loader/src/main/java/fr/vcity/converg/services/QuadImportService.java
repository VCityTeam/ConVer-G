...
public class QuadImportService implements IQuadImportService {
    ...
    public Integer importModel(File file) throws RiotException {
        ...
        try (FileInputStream inputStream = new FileInputStream(file)) {
            ...
            log.info("[Measure] (Catalog): {} ns for file: {};", endCatalog - startCatalog, file.getName());
            ...
            log.info("[Measure] (Condensing): {} ns for file: {};", endCondensing - startCondensing, file.getName());

            flatModelQuadRepository.deleteAll();

            Long end = System.nanoTime();
            log.info("[Measure] (Import relational internal): {} ns for file: {};", end - start, file.getName());
     ...
     */
    @Override
    public void importMetadata(File file) throws RiotException, FileNotFoundException {
        log.info("Current file: {}", file.getName());

        try (FileInputStream inputStream = new FileInputStream(file)) {
            Long start = System.nanoTime();

            getTriplesStreamRDF(inputStream, file.getName())
                    .finish();
