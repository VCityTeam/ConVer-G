...
class EntailmentSQLTranslationTest {
    ...
    @Test
    void schemaPremiseIsNotConstrainedToTheDataGraph() {
        // The subClassOf closure must carry its own graph coordinate (so an
        // ontology shipped in a dedicated named graph is matched) and the data
        // graph's version set must be intersected with the schema graph's one
        String sparql = """
                PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
                SELECT ?g ?x ?t WHERE { GRAPH ?g { ?x rdf:type ?t } }
                """;
        ...
        EntailmentRewriter rewriter = new EntailmentRewriter(RDFSRules.allRules());
        Op rewrittenOp = rewriter.rewrite(quadOp);
        String sql = condensedRdfs.buildSPARQLContext(rewrittenOp).getSql();

        assertTrue(sql.contains("bs$__ent_sg"),
                "Schema premise should carry its own version-set column");
        assertTrue(sql.matches("(?s).*bit_count\\(gvi_table\\.bs\\$g & gvi_table\\.bs\\$__ent_sg_\\d+\\) <> 0.*"),
                "Data and schema version sets should be intersected");
    }
