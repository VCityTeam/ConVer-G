...
class EntailmentRewriterTest {
    ...
    @Test
    void schemaPremiseHasItsOwnGraphScope() {
        // The schema pattern must be scoped by a fresh graph variable (so a TBox in
        // a dedicated ontology graph is found) and version-intersected with the data
        // graph through OpGraphVersionIntersection
        Op op = parseToQuadOp(
                "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\n" +
                "SELECT ?g ?x ?t WHERE { GRAPH ?g { ?x rdf:type ?t } }");
        Op rewritten = rdfsRewriter.rewrite(op);

        List<OpGraphVersionIntersection> intersections = collectOps(rewritten, OpGraphVersionIntersection.class);
        assertFalse(intersections.isEmpty(),
                "Rule expansions should be wrapped in OpGraphVersionIntersection");
        for (OpGraphVersionIntersection gvi : intersections) {
            assertTrue(gvi.getSchemaGraphVar().getName().startsWith("__ent_"),
                    "Schema graph scope should be a rewriter-allocated variable");
            assertNotEquals(gvi.getDataGraphNode(), gvi.getSchemaGraphVar(),
                    "Schema graph scope must differ from the data graph scope");
        }
    }

    @Test
    void unionBranchesProjectTheSameVariables() {
        // Expansions bind extra __ent_* variables; every union branch must be
        // projected onto the same visible variables so the SQL UNION columns align
        Op op = parseToQuadOp(
                "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\n" +
                "SELECT ?g ?x ?t WHERE { GRAPH ?g { ?x rdf:type ?t } }");
        Op rewritten = rdfsRewriter.rewrite(op);

        List<OpUnion> unions = collectOps(((OpProject) rewritten).getSubOp(), OpUnion.class);
        assertFalse(unions.isEmpty(), "Rewriting should produce unions");
        for (OpUnion union : unions) {
            for (Op branch : List.of(union.getLeft(), union.getRight())) {
                Op unwrapped = branch instanceof OpUnion ? branch : requireProject(branch);
                if (unwrapped instanceof OpProject project) {
                    assertEquals(List.of("g", "x", "t"),
                            project.getVars().stream().map(Node_Variable::getName).toList(),
                            "Each union branch must project the quad's visible variables");
                }
            }
        }
    }

    private static Op requireProject(Op branch) {
        assertInstanceOf(OpProject.class, branch,
                "Union branches of an entailment expansion must be projected");
        return branch;
    }
    ...
    /**
     * Recursively collect all nodes of the given type in an Op tree.
     */
    private static <T extends Op> List<T> collectOps(Op op, Class<T> type) {
        List<T> collected = new java.util.ArrayList<>();
        collectOps(op, type, collected);
        return collected;
    }

    private static <T extends Op> void collectOps(Op op, Class<T> type, List<T> collected) {
        if (type.isInstance(op)) collected.add(type.cast(op));
        switch (op) {
            case Op2 op2 -> {
                collectOps(op2.getLeft(), type, collected);
                collectOps(op2.getRight(), type, collected);
            }
            case Op1 op1 -> collectOps(op1.getSubOp(), type, collected);
            case OpGraphVersionIntersection opGVI -> collectOps(opGVI.getSubOp(), type, collected);
            default -> {
            }
        }
    }
    ...
    private static boolean containsOpType(Op op, Class<? extends Op> type) {
        ...
        return switch (op) {
            ...
            case OpGraphVersionIntersection opGVI -> containsOpType(opGVI.getSubOp(), type);
            default -> false;
        };
    }
}
