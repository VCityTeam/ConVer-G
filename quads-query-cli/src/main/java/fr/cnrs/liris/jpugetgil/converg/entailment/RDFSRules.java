 ...
 * <p>
 * The schema premise (subClassOf/subPropertyOf chain, domain or range declaration) is
 * matched in its own graph scope — a fresh variable — so a TBox shipped in a dedicated
 * ontology named graph is found even though the data lives in other named graphs. The
 * expansion is wrapped in an {@link OpGraphVersionIntersection} so versioned semantics
 * are preserved: the inferred triple only holds in versions where both the data premise
 * (in its graph) and the schema premise (in its graph) exist.
 */
public final class RDFSRules {
     ...
     * <pre>
     *   OpGraphVersionIntersection(
     *     OpJoin(
     *       OpQuadPattern(graph, [?x rdf:type ?__ent_sub]),
     *       OpTransitiveClosure(?__ent_sg, ?__ent_sub, rdfs:subClassOf, ?C)
     *     ),
     *     graph, ?__ent_sg
     *   )
     * </pre>
     * where ?C is the original object (the superclass we're looking for) and
     * ?__ent_sg is the schema graph scope (any named graph, e.g. a dedicated
     * ontology graph). The intersection computes version-set conjunction: the
     * inferred type only holds in versions where both the type assertion and
     * the subClassOf chain exist.
     */
    public static EntailmentRule rdfs9() {
        return new EntailmentRule() {
            ...
            public Op expand(Quad quad, Node graphNode, FreshVariableAllocator allocator) {
                ...
                // Fresh variables for the intermediate subclass and the schema graph
                Var subClass = allocator.allocate("sub");
                Var schemaGraph = allocator.allocate("sg");
                ...
                // Transitive closure: ?__ent_sub rdfs:subClassOf+ ?C (in any graph)
                Op closureOp = new OpTransitiveClosure(schemaGraph, subClass, object, RDFS_SUBCLASS_OF);

                return new OpGraphVersionIntersection(
                        OpJoin.create(typeOp, closureOp), graphNode, schemaGraph);
            }
        };
    }
    ...
    public static EntailmentRule rdfs7() {
        return new EntailmentRule() {
            ...
            public Op expand(Quad quad, Node graphNode, FreshVariableAllocator allocator) {
                ...
                // Fresh variables for the sub-property and the schema graph
                ...
                // Transitive closure: ?__ent_subp rdfs:subPropertyOf+ ?p (in any graph)
                Op closureOp = new OpTransitiveClosure(schemaGraph, subProp, predicate, RDFS_SUBPROPERTY_OF);

                return new OpGraphVersionIntersection(
                        OpJoin.create(dataOp, closureOp), graphNode, schemaGraph);
            }
        };
    }
    ...
    public static EntailmentRule rdfs2() {
        return new EntailmentRule() {
            ...
            public Op expand(Quad quad, Node graphNode, FreshVariableAllocator allocator) {
                ...
                // Pattern: ?__ent_dp rdfs:domain ?C (in any graph)
                BasicPattern domainPattern = new BasicPattern();
                domainPattern.add(Triple.create(freshPred, RDFS_DOMAIN_NODE, classNode));
                Op domainOp = new OpQuadPattern(schemaGraph, domainPattern);

                return new OpGraphVersionIntersection(
                        OpJoin.create(dataOp, domainOp), graphNode, schemaGraph);
            }
        };
    }
    ...
    public static EntailmentRule rdfs3() {
        return new EntailmentRule() {
            ...
            public Op expand(Quad quad, Node graphNode, FreshVariableAllocator allocator) {
                ...
                // Pattern: ?__ent_rp rdfs:range ?C (in any graph)
                BasicPattern rangePattern = new BasicPattern();
                rangePattern.add(Triple.create(freshPred, RDFS_RANGE_NODE, classNode));
                Op rangeOp = new OpQuadPattern(schemaGraph, rangePattern);

                return new OpGraphVersionIntersection(
                        OpJoin.create(dataOp, rangeOp), graphNode, schemaGraph);
            }
        };
    }
