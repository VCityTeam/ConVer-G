...
FROM versioned_quad vq1
         ...
         JOIN resource_or_literal rl2 ON vq1.id_predicate = rl2.id_resource_or_literal AND rl2.name = 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type'
         ...
         JOIN resource_or_literal rl4 ON vq2.id_predicate = rl4.id_resource_or_literal AND rl4.name = 'http://www.opengis.net/citygml/transportation/2.0/transportation#Stop.accessibility'
         JOIN resource_or_literal rl5 ON vq2.id_object = rl5.id_resource_or_literal AND rl5.name = 'true'
         JOIN version ve ON (ve.transaction_time_start, COALESCE(ve.transaction_time_end, NOW())) OVERLAPS (date '2021-05-04 00:00', date '2023-08-23 00:00');
