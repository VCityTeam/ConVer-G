-- # Query the metadata (default graph) for all triples
SELECT rl1.name as "s", rl2.name as "p", rl3.name as "o" -- récupération des noms des ressources (URI ou litéral)
FROM (
     SELECT t1.id_subject as w$s, t1.id_predicate as w$p, t1.id_object as w$o
     FROM metadata t1 -- contexte de triple (métadonnées)
 ) sq1
