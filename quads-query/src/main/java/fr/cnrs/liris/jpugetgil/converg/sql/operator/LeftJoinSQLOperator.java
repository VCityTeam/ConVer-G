...
public class LeftJoinSQLOperator extends JoinSQLOperator {
        ...
        private String buildDifferenceGroupBy() {
                return "GROUP BY " + leftQuery.getContext().sparqlVarOccurrences()
                                .keySet()
                                .stream()
                                .map((node) -> SQLUtils.getMaxSPARQLOccurrence(
                                                leftQuery.getContext().sparqlVarOccurrences().get(node)))
         ...
         * @param sliceIndex  the index of the coordinate carrying the {@code A_i \ B_i}
         *                    factor
        ...
        private String buildDifferenceSliceSelect(List<Node> orderedZeta, int sliceIndex) {
                return "SELECT " + mergedMapOccurrences
                                ...
                                .map((node) -> {
                                        ...
                                        return leftVar.leftJoinDifferenceSliceProjection(rightVar, LEFT_TABLE_NAME,
                                                        RIGHT_TABLE_NAME, role);
                                })
                                .collect(Collectors.joining(", ")) + "\n";
        }

        /**
         * The shared condensed (ζ) coordinates of the left join, in a fixed (name)
         * order.
         * These are exactly the coordinates that carry a version-set difference; the
         * order
        ...
        private boolean isSharedCondensed(Node node) {
                ...
                return left != null && right != null
                                && SQLUtils.getMaxSPARQLOccurrence(left).getSqlVariable()
                                                .getSqlVarType() == SQLVarType.CONDENSED;
        }
        ...
        protected String buildFrom() {
                ...
                return " FROM " + LEFT_TABLE_NAME + " LEFT JOIN " + RIGHT_TABLE_NAME + " ON "
                                + sqlJoinClauseBuilder.build().clause;
        }
        ...
        protected String buildDifferenceFrom() {
                ...
                return " FROM " + LEFT_TABLE_NAME + " JOIN " + RIGHT_TABLE_NAME + " ON "
                                + sqlJoinClauseBuilder.build().clause;
        }
         ...
         * (left.bs &amp; ~right.bs) and only makes sense when the OPTIONAL is
         * correlated
         * through a shared CONDENSED (graph) variable. When the two sides are joined
         * only
         * through ID variables (e.g. two distinct graph variables connected by a shared
         * subject), the plain LEFT JOIN already yields the correct OPTIONAL semantics
         * and
         * the difference branch would be both redundant and malformed.
         *
         * @return true if at least one common variable is in the CONDENSED
         *         representation
         */
